...
def get_parser(allow_policy_list=False):
    ...
    parser.add_argument(
        '--env-kwargs',
        type=lambda kwargs: json.loads(kwargs.replace("\\quote", "\"")), 
        default='{}',
        help="override kwargs for training and eval envs")

    parser.add_argument(
        '--eval-env-kwargs',
        type=lambda kwargs: json.loads(kwargs.replace("\\quote", "\"")), 
        default='{}',
        help="override kwargs for eval env only, applied after env-kwargs")
